...
- **📋 データのコピーと提出**:
  ...
  2. Google フォームの回答欄に戻り、コピーしたデータを貼り付けて送信してください。
...
### 📁 保存済みデータ（CSV）から結果を再表示する方法
![CSVビューアー画面](images/step5_csv_viewer.png)

Google フォームで回収した回答テキストや、ダウンロード保存した `.csv` ファイルを読み込ませることで、**実験直後の結果画面（正解数・正答率・平均反応時間・全問の正誤テーブル）をブラウザ上でいつでも完全再現**できます。

#### 活用シーン
- **研究者・実験者**: Google フォームのスプレッドシートに集まった参加者の回答データから、個別の正誤状況や反応時間を視覚的に確認・検証したいとき
- **参加者**: 保存したCSVファイルから、後から自分の結果ダッシュボードを再確認したいとき

#### 操作手順
1. トップ（スタート）画面下部にある **「📁 保存済みデータ (CSV) を読み込む」** ボタンをクリックします。
2. 表示されたダイアログで、以下のいずれかの方法でデータを入力します：
   - **テキスト貼り付け**: テキストエリアに Google フォーム等の回答CSVテキストを貼り付け
   - **ファイル選択**: **「📂 CSVファイルを選択」** ボタンから保存済みの `.csv` ファイルを選択
3. **「結果を表示する」** ボタンをクリックすると、該当参加者の結果ダッシュボードが即座に再現・表示されます。
...
4. **保存済みデータの復元・再表示（CSVビューアー機能）**
   - Google フォームの回答テキストやCSVファイルを読み込むだけで、正答率・平均反応時間・全12問の正誤一覧テーブルをいつでもグラフィカルに再現できます。
...
4. **データの確認・集計 (実験後)**: 集まった回答テキストを本アプリのCSVビューアーに貼り付けて正誤一覧を個別確認、またはスプレッドシート/R/Python等で一括集計。
